						...
						switch($val)
                        {
						    case "q":
						        ...
						        case "h":
									...
									elseif($h <= 900)
									{$h_min=$h-50;$h_max=$h+50;}
									elseif($h<=1000){$h_min=$h-100;$h_max=$h+100;}
									elseif($h<=1100){$h_min=1000;$h_max=1100;}
									else{$err_h=1;unset($h);}
									
									if(!isset($err_h))
									{
										$rh =1 ;
										$search_prod_tmp[]  = 'h';
										$hid_line_finder   .= "<input type='hidden' value='".$h."' name='h' id='h'>";
									}
									break;
								...
								case "w":
									...
									{$w_min=$w-4;$w_max=$w+4;}
									elseif($w<=45)
									{$w_min=$w-10;$w_max=$w+10;}
									elseif($w<=135)
									{$w_min=$w-25;$w_max=$w+25;}
									elseif($w<=315)
									{$w_min=$w-85;$w_max=$w+85;}
									elseif($w<=500)
									{$w_min=$w-100;$w_max=$w+100;}
									elseif($w<=630)
									{$w_min=500;$w_max=630;}
									else
									{$err_w=1;unset($w);}
									...
									if(!isset($err_w))
									{
										$rw=1;
										$search_prod_tmp[]='w';
										$hid_line_finder.="<input type='hidden' value='".$w."' name='w' id='w'>";
									}
									break;
								...
								case "kav":
									...
									if(!isset($err_kav))
									{
										...
										$search_prod_tmp[]  = 'kav';
										$hid_line_finder   .=
											"<input type = 'hidden' value = '".$kav."' name = 'kav' id = 'kav'>";
									}
									break;
								...
								case "p":
									...
									{$p_min=0;$p_max=0.3;}
									elseif($p<0.8)
									{$p_min=$p-0.1;$p_max=$p+0.1;}
									elseif($p<=1.2)
									{$p_min=$p-0.2;$p_max=$p+0.2;}
									elseif($p<=1.7)
									{$p_min=$p-0.1;$p_max=$p+0.1;}
									elseif($p<=2.4)
									{$p_min=$p-0.7;$p_max=$p+0.7;}
									elseif($p<=3)
									{$p_min=$p-0.5;$p_max=$p+0.5;}
									elseif($p<=4)
									{$p_min=3.2;$p_max=4;}
									else
									{$err_p=1;unset($p);}
									...
									if(!isset($err_p))
									{
										$rp=1;
										$search_prod_tmp[]='p';
										$hid_line_finder.="<input type='hidden' value='".$p."' name='p' id='p'>";
									}
									break;
								case "t":
									if($t<=80)
